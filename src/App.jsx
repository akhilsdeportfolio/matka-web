...
import { Button, Skeleton, SpinLoading } from 'antd-mobile'
...
export default function App () {
  ...
  useEffect(() => {
    //const gameChannel = client.channels.get('game')
  ...
  return (
    <AuthProvider>
      ...
      <h1 className='text-3xl font-bold text-red-500 text-center'>{val}x</h1>
      <Button disabled={disable} className='p-4 bg-orange-400' type='ghost' onClick={()=>{setDisabled((val)=>!val);console.log(val);coin.play()}}>Cash Out {formatMoneyWithDecimals.format(100*val)}</Button>      
      </div> */}
      <Router>
